  ...
  export default {
    ...
    methods: {
      init (id) {
        ...
        this.$http({
          url: this.$http.adornUrl('/api/v1/menu/list'),
          method: 'get',
          params: this.$http.adornParams()
        }).then(({data}) => {
          this.menuList = treeDataTranslate(data.menus, 'menuId')
        }).then(() => {
          this.visible = true
          this.$nextTick(() => {
          ...
          if (!this.dataForm.id) {
            ...
            // 修改
            this.$http({
              url: this.$http.adornUrl(`/api/v1/menu/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/api/v1/menu/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
